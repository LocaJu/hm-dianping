...
//    @Autowired
    ...
    public Result login2(LoginFormDTO loginForm, HttpSession session) {
        ...
        String code = loginForm.getCode();
        //从redis中获取验证码
        String imageCheckCode = stringRedisTemplate.opsForValue().get(IMAGE_LOGIN_CODE_KEY);
        if (imageCheckCode == null || !imageCheckCode.equalsIgnoreCase(code)) {
            // 不一致，报错
            return Result.fail("验证码出错");
        }
...
//        stringRedisTemplate.opsForSet().add(IMAGE_LOGIN_CODE_KEY,checkCode);
        stringRedisTemplate.opsForValue().set(IMAGE_LOGIN_CODE_KEY,checkCode,LOGIN_CODE_TTL,TimeUnit.MINUTES);
        return Result.ok();
    }
